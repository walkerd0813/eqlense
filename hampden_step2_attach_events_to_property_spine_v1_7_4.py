# hampden_step2_attach_events_to_property_spine_v1_7_4.py
...
GOOD_ADDR_KEY_PATTERNS = [
    ...
    r"\bfull_address\b", r"\baddress_line1\b", r"\bline1\b"
]
...
def address_score(path, key, val):
    ...
    if kl.endswith("state") or kl == "state": return -999
    if "zipcode" in kl or kl == "zip": return -999
    if "postal" in kl: return -999

    if v.upper() in ("MA","MASSACHUSETTS"): return -999
    if not re.search(r"\b\d{1,6}\b", v): return -50

    score = 0
    if key_matches_any(kl, GOOD_ADDR_KEY_PATTERNS): score += 50
    ...
    pl = path.lower()
    if "assessor" in pl or "situs" in pl or "address" in pl: score += 10
    return score
...
def collect_event_towns(events_dir):
    towns=set()
    for fn in os.listdir(events_dir):
        ...
        with open(os.path.join(events_dir, fn), "r", encoding="utf-8") as f:
            for line in f:
                ...
                t = pr.get("town") or e.get("town_raw") or ""
                t = norm(t)
                if t:
                    # strip trailing junk tokens that sometimes appear
                    t = re.sub(r"\s+ADDR$", "", t).strip()
                    if t:
                        towns.add(t)
    return towns

def build_spine_index(spine_path, allowed_towns=None):
    idx={}
    rows_seen=0
    rows_indexed=0
    samples=[]
    ...
    for rec in iter_spine_records(spine_path):
        ...
        town_n = re.sub(r"\s+ADDR$", "", town_n).strip()

        if allowed_towns is not None and town_n not in allowed_towns:
            continue
...
def main():
    ...
    allowed_towns = collect_event_towns(a.eventsDir)
    spine_idx, spine_rows_seen, spine_rows_indexed, spine_samples = build_spine_index(a.spine, allowed_towns=allowed_towns)
            ...
            with open(src,"r",encoding="utf-8") as f:
                for line in f:
                    ...
                    if not town_n or not addr_n:
                        ...
                        if len(missing_locator_samples) < 15:
