...
$SCRIPT = Join-Path $ROOT "scripts\phase4_assessor\phase4_assessor_inventory_harvest_v2.mjs"

if (-not (Test-Path $CONFIG)) { throw ("[err] Missing config: {0}" -f $CONFIG) }
if (-not (Test-Path $SCRIPT)) { throw ("[err] Missing node script: {0}" -f $SCRIPT) }

Say "[start] Phase 4 Assessor — inventory + harvest (v2)"
