...
function buildHistoricIndex(attachmentsPath, mapping) {
  ...
  const defaultReview = mapping?.enforceable?.default_review_required_flag ?? true;
  const defaultExterior = mapping?.enforceable?.default_exterior_change_review_flag ?? true;
...
async function main() {
  ...
  if (!contractIn) throw new Error('Missing --contractIn');
  if (!attachments) throw new Error('Missing --attachments');
  if (!out) throw new Error('Missing --out');
  if (!mappingPath) throw new Error('Missing --mapping');

  if (!fs.existsSync(contractIn) || fs.statSync(contractIn).isDirectory()) {
    throw new Error(`--contractIn must be an existing FILE: ${contractIn}`);
  }
