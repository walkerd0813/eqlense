# hampden_step2_attach_events_to_property_spine_v1_7_3.py
import json, argparse, os, re
from datetime import datetime

def norm(s):
    if not isinstance(s, str): return ""
    s = s.strip().upper()
    s = re.sub(r"\s+", " ", s)
    # clean common junk tokens that showed up in your audit ("SPRINGFIELD ADDR", trailing " Y")
    s = re.sub(r"\bADDR\b", "", s).strip()
    s = re.sub(r"\s+", " ", s).strip()
    return s

def norm_addr(s):
    if not isinstance(s, str): return ""
    s = s.strip().upper()
    s = re.sub(r"\s+", " ", s)
    # remove single trailing status flag like " Y"
    s = re.sub(r"\s+Y$", "", s).strip()
    return s

EXCLUDE_ADDR_KEYS = set([
    "state","st","province","county","country","zip","zipcode","postal","post_code","lat","lon","lng",
    "parcel_id","map","lot","block"
])

# address keys we actually want
GOOD_ADDR_KEY_PATTERNS = [
    r"\baddress\b", r"\baddr\b", r"\bsitus\b", r"\bsite_address\b", r"\bstreet_address\b",
    r"\bfull_address\b", r"\baddress_line1\b", r"\bline1\b", r"\bmailing_address\b"
]

TOWN_KEY_PATTERNS = [r"\btown\b", r"\bcity\b", r"\bmunicipality\b", r"\blocality\b"]

def iter_kv(obj, path="", depth=0, max_depth=6):
    if depth > max_depth: return
    if isinstance(obj, dict):
        for k, v in obj.items():
            p = f"{path}.{k}" if path else k
            yield (p, k, v)
            yield from iter_kv(v, p, depth+1, max_depth)
    elif isinstance(obj, list):
        for i, v in enumerate(obj[:50]):
            p = f"{path}[{i}]"
            yield (p, str(i), v)
            yield from iter_kv(v, p, depth+1, max_depth)

def key_matches_any(kl, patterns):
    return any(re.search(p, kl) for p in patterns)

def address_score(path, key, val):
    # higher is better
    if not isinstance(val, str): return -999
    v = val.strip()
    if not v: return -999

    kl = key.lower().strip()

    # hard exclude common non-address fields
    if kl in EXCLUDE_ADDR_KEYS: return -999
    if kl.endswith("state") or "state" == kl: return -999
    if "zipcode" in kl or kl == "zip": return -999
    if "postal" in kl: return -999

    # must look like an address (have a number)
    if not re.search(r"\b\d{1,6}\b", v): 
        return -50

    # reject pure state abbrev
    if v.upper() in ("MA","MASSACHUSETTS"): 
        return -999

    score = 0

    # prefer good key names
    if key_matches_any(kl, GOOD_ADDR_KEY_PATTERNS):
        score += 50

    # prefer strings that look like street names
    if re.search(r"\b(ST|STREET|RD|ROAD|AVE|AVENUE|DR|DRIVE|LN|LANE|BLVD|WAY|CT|CIR|PL)\b", v.upper()):
        score += 25

    # length heuristic
    if len(v) >= 10: score += 10
    if len(v) >= 20: score += 10

    # path bonus if nested under assessor/situs style branches
    pl = path.lower()
    if "assessor" in pl or "situs" in pl or "address" in pl:
        score += 10

    return score

def town_score(path, key, val):
    if not isinstance(val, str): return -999
    v = val.strip()
    if not v: return -999
    kl = key.lower().strip()
    score = 0
    if key_matches_any(kl, TOWN_KEY_PATTERNS): score += 50
    # reject obvious non-towns
    if v.upper() in ("MA","MASSACHUSETTS","USA","UNITED STATES"): return -999
    # town should be alphabetic-ish
    if re.search(r"\d", v): score -= 20
    if len(v) >= 3: score += 5
    return score

def pick_best_town_addr(rec):
    best_t = None
    best_a = None

    for p, k, v in iter_kv(rec):
        if isinstance(v, str):
            ts = town_score(p, k, v)
            if ts > (best_t[0] if best_t else -999):
                best_t = (ts, p, k, v)

            a_s = address_score(p, k, v)
            if a_s > (best_a[0] if best_a else -999):
                best_a = (a_s, p, k, v)

    return best_t, best_a

def iter_spine_records(spine_path):
    # try NDJSON first
    try:
        with open(spine_path, "r", encoding="utf-8") as f:
            first = f.readline()
            if first:
                try:
                    json.loads(first)
                    yield json.loads(first)
                    for line in f:
                        line=line.strip()
                        if not line: continue
                        yield json.loads(line)
                    return
                except Exception:
                    pass
    except Exception:
        pass

    with open(spine_path, "r", encoding="utf-8") as f:
        obj = json.load(f)

    # meta wrapper -> properties_ndjson
    if isinstance(obj, dict) and isinstance(obj.get("properties_ndjson"), str):
        nd = obj["properties_ndjson"]
        if not os.path.isabs(nd):
            nd = os.path.normpath(os.path.join(os.path.dirname(spine_path), nd))
        with open(nd, "r", encoding="utf-8") as f2:
            for line in f2:
                line=line.strip()
                if not line: continue
                yield json.loads(line)
        return

    if isinstance(obj, list):
        for rec in obj:
            if isinstance(rec, dict):
                yield rec
        return

    if isinstance(obj, dict):
        yield obj

def build_spine_index(spine_path, limit_scan=600000):
    idx={}
    rows_seen=0
    rows_indexed=0
    samples=[]

    for rec in iter_spine_records(spine_path):
        rows_seen += 1
        pid = rec.get("property_id") or rec.get("id") or rec.get("propertyId")
        if not pid:
            continue

        best_t, best_a = pick_best_town_addr(rec)
        if not best_t or not best_a:
            continue

        town_raw = best_t[3]
        addr_raw = best_a[3]

        town_n = norm(town_raw)
        addr_n = norm_addr(addr_raw)

        if not town_n or not addr_n:
            continue

        # final safety: address must contain a number
        if not re.search(r"\b\d{1,6}\b", addr_n):
            continue

        k = f"{town_n}|{addr_n}"
        idx[k]=pid
        rows_indexed += 1

        if len(samples) < 8:
            samples.append({
                "property_id": pid,
                "town_path": best_t[1],
                "town_key": best_t[2],
                "town_raw": town_raw,
                "addr_path": best_a[1],
                "addr_key": best_a[2],
                "addr_raw": addr_raw,
                "key": k,
                "town_score": best_t[0],
                "addr_score": best_a[0],
            })

        if rows_seen >= limit_scan and len(idx) > 0:
            break

    return idx, rows_seen, rows_indexed, samples

def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--eventsDir", required=True)
    ap.add_argument("--spine", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--audit", required=True)
    a=ap.parse_args()

    os.makedirs(os.path.dirname(a.out), exist_ok=True)

    spine_idx, spine_rows_seen, spine_rows_indexed, spine_samples = build_spine_index(a.spine)

    counts={"ATTACHED_A":0,"UNKNOWN":0,"MISSING_TOWN_OR_ADDRESS":0}
    missing_locator_samples=[]
    unmatched_samples=[]

    with open(a.out,"w",encoding="utf-8") as out:
        for fn in os.listdir(a.eventsDir):
            if not fn.endswith(".ndjson"): continue
            src=os.path.join(a.eventsDir, fn)
            with open(src,"r",encoding="utf-8") as f:
                for line in f:
                    line=line.strip()
                    if not line: continue
                    e=json.loads(line)

                    pr=e.get("property_ref") or {}
                    town = pr.get("town") or e.get("town_raw") or ""
                    addr = pr.get("address") or e.get("address_raw") or ""

                    town_n = norm(town)
                    addr_n = norm_addr(addr)

                    if not town_n or not addr_n:
                        counts["UNKNOWN"] += 1
                        counts["MISSING_TOWN_OR_ADDRESS"] += 1
                        if len(missing_locator_samples) < 10:
                            missing_locator_samples.append({"src":fn,"event_id":e.get("event_id"),"event_type":e.get("event_type"),"town_raw":town,"address_raw":addr})
                        e["attach_status"]="UNKNOWN"
                        out.write(json.dumps(e)+"\n")
                        continue

                    key=f"{town_n}|{addr_n}"
                    pid=spine_idx.get(key)

                    if pid:
                        e["property_id"]=pid
                        e["attach_status"]="ATTACHED_A"
                        counts["ATTACHED_A"] += 1
                    else:
                        e["attach_status"]="UNKNOWN"
                        counts["UNKNOWN"] += 1
                        if len(unmatched_samples) < 10:
                            unmatched_samples.append({"src":fn,"event_id":e.get("event_id"),"event_type":e.get("event_type"),"town_norm":town_n,"address_norm":addr_n})

                    out.write(json.dumps(e)+"\n")

    os.makedirs(os.path.dirname(a.audit), exist_ok=True)
    with open(a.audit,"w",encoding="utf-8") as af:
        json.dump({
            "created_at": datetime.utcnow().replace(microsecond=0).isoformat() + "Z",
            "events_dir": a.eventsDir,
            "spine_path": a.spine,
            "spine_rows_seen": spine_rows_seen,
            "spine_rows_indexed": spine_rows_indexed,
            "spine_index_keys": len(spine_idx),
            "counts": counts,
            "samples": {
                "spine_key_examples": spine_samples,
                "missing_locator": missing_locator_samples,
                "unmatched_locator": unmatched_samples
            }
        }, af, indent=2)

    print("[done] spine_rows_seen:", spine_rows_seen)
    print("[done] spine_index_keys:", len(spine_idx))
    print("[done] spine_rows_indexed:", spine_rows_indexed)
    print("[done] attach_status_counts:", counts)
    print("[done] audit:", a.audit)
    print("[done] out:", a.out)

if __name__=="__main__":
    main()
