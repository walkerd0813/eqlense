...
# If user passed a single args line, split it into CmdArgs
...
# Parse, but guard: if Gatekeeper emits non-standard JSON, print it and fail clearly
$gateJson = $null
try { $gateJson = $gateOut | ConvertFrom-Json } catch { $gateJson = $null }

if (-not $gateJson -or -not ($gateJson.PSObject.Properties.Name -contains "overall")) {
  Write-Host $gateOut
  throw "[error] gatekeeper returned unexpected output (engine_id may be missing from registry or Gatekeeper errored)"
}
...
# Use Process to enforce timeout (PS5.1-safe)
...
if(-not $finished){
  try { $p.Kill() } catch {}
  throw ("[error] engine timeout after {0}s" -f $TimeoutSec)
}
...
Add-Content -Path $jr -Value $entry
