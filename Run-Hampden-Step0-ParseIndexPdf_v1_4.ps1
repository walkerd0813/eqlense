...
Write-Host '[start] Hampden STEP 0 v1.4 — Parse Index PDF (NO OCR, NO ATTACHING)'
...
$Script = Join-Path $PSScriptRoot 'parse_hampden_index_pdf_v1_4.py'
...
function Pick-Pdf {
  ...
  if ($explicitPath -and (Test-Path -LiteralPath $explicitPath)) { return Get-Item -LiteralPath $explicitPath }
  ...
  if ($regex) {
    try {
      ...
      if ($hit) { return $hit }
    } catch { Write-Host ('[warn] invalid -NameRegex: {0}' -f $regex) }
  }
...
$Audit  = Join-Path $BackendRoot 'publicData\_audit\registry\hampden_index_raw_v1_4_audit.json'
...
$appendFlag = ''
if ($Append) { $appendFlag = '--append' ; Write-Host '[info] append: true' }
...
Write-Host '[done] Hampden STEP 0 v1.4 complete.'
