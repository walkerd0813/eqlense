...
Write-Host "[start] Hampden STEP 2 v1.6 - Attach events to Property Spine (confidence-gated)" -ForegroundColor Cyan
...
$outDir = Join-Path $BackendRoot "publicData\registry\hampden\_attached_v1_6"
New-Item -ItemType Directory -Force -Path $outDir | Out-Null
$out = Join-Path $outDir "events_attached_v1_6.ndjson"

$auditDir = Join-Path $BackendRoot "publicData\_audit\registry"
New-Item -ItemType Directory -Force -Path $auditDir | Out-Null
$audit = Join-Path $auditDir "hampden_step2_attach_audit_v1_6.json"

$py = Join-Path $BackendRoot "hampden_step2_attach_events_to_property_spine_v1_6.py"
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "[error] Step 2 v1.6 failed." -ForegroundColor Red
  exit $LASTEXITCODE
}

Write-Host "[done] Hampden STEP 2 v1.6 complete." -ForegroundColor Green
Write-Host "[next] Open audit JSON: hampden_step2_attach_audit_v1_6.json and check ATTACHED_A vs UNKNOWN." -ForegroundColor Yellow
