...
function computePropertyUid(o){
  const town = normTown(o?.town);
  const parcel = String(o?.parcel_id ?? "").trim();
  if(!town || !parcel) return null;
  if(parcel.toUpperCase() === "UNKNOWN") return null;
  if(o?.id_quality === "PLACEHOLDER_UNKNOWN") return null;
  return `${town}|${parcel}`;
}

// Must match addPropertyUid_rowUid_v1.mjs basis so keys line up
function computeRowUid(o){
  const property_uid = o?.property_uid ?? computePropertyUid(o) ?? "(null)";
  const rowBasis = [
    property_uid,
    String(o?.unit ?? ""),
  ...
  return sha1(rowBasis);
}

function getPatchKey(o){
  // prefer precomputed
  if (o?.row_uid) return o.row_uid;
  return computeRowUid(o);
}

function isPlaceholderUnknown(o){
  const parcel = String(o?.parcel_id ?? "").trim().toUpperCase();
  return parcel === "UNKNOWN" || o?.id_quality === "PLACEHOLDER_UNKNOWN";
}
F
...
const inLonRange = (v)=> Number.isFinite(v) && v <= -69.0 && v >= -74.5;
const inLatRange = (v)=> Number.isFinite(v) && v >= 40.5 && v <= 44.5;
...
function parseLonLatFromFilename(fn){
  ...
  const rawA = Number(nums[0]);
  const rawB = Number(nums[1]);
  ...
  for(const [a, b] of candidates){
    if(inLonRange(a) && inLatRange(b)) return { lon: a, lat: b };
    if(inLonRange(b) && inLatRange(a)) return { lon: b, lat: a };
  }

  // fallback (still deterministic)
  return { lon: rawA, lat: rawB };
}
...
console.log("Sample tile keys:", Array.from(tileMap.keys()).slice(0,5));
...
function extractMadFields(m){
  const lon = Number(pick(m, ["lon","lng","x","X","LONGITUDE","Longitude","LONG","LONG_","X_COORD"]));
  const lat = Number(pick(m, ["lat","y","Y","LATITUDE","Latitude","LAT","LAT_","Y_COORD"]));
  const town = pick(m, ["town","TOWN","municipality","MUNICIPALITY","MUNI","MUNI_NAME","city","CITY"]);
  ...
  return { lon, lat, town, zip, streetNo, streetName };
}
...
for await (const line of rlQ){
  ...
  targets.set(id, { lon, lat, x, y });
}
console.log("Targets loaded from quarantine:", { qTotal, qParseErr, targetIds: targets.size });

// ---------------- Compute patches by nearest + Town-PIP accept ----------------
let noXY=0, noLatLon=0, noPipTown=0, noTile=0, tooFar=0, townMismatch=0, noCandidate=0, autoAccept=0;
...
for(const [id, t] of targets.entries()){
  ...
  for(let dx=-1; dx<=1; dx++){
    for(let dy=-1; dy<=1; dy++){
      const lonK = (baseLonKey + dx*tileSize).toFixed(2);
      const latK = (baseLatKey + dy*tileSize).toFixed(2);
      const k = `${lonK}|${latK}`;
  ...
  if(!candTown || !best.mad.streetName){ noCandidate++; continue; }
