...
/**
 * SAMPLE BUCKET RECORDS (NDJSON streaming) - v2 DROPIN
 * ----------------------------------------------------
 * Like v1, but adds optional regex filters for parcel_id, property_id, and full_address.
 *
 * Examples:
 *   node .\mls\scripts\sampleBucket_v2_DROPIN.js --in C:\...\file.ndjson --bucket noStreetNo --limit 20
 *   node .\mls\scripts\sampleBucket_v2_DROPIN.js --in C:\...\file.ndjson --bucket noStreetNo --limit 20 --parcelRegex "^\d{1,5}-"
 *   node .\mls\scripts\sampleBucket_v2_DROPIN.js --in C:\...\file.ndjson --bucket noStreetNo --limit 20 --addrRegex "#|UNIT|APT"
 */
...
function isBucket(o, bucket) {
  ...
  const fullU = U(full);
  const nameU = U(streetName);

  switch (bucket) {
    case "noStreetNo":
      return streetNo === "" || streetNo === "null";
    case "streetNoZero":
      return streetNo === "0";
    case "lotLike":
...
async function main() {
  ...
  const parcelRegexRaw = getArg("parcelRegex", "");
  const propRegexRaw = getArg("propertyRegex", "");
  const addrRegexRaw = getArg("addrRegex", "");
  const townFilter = U(getArg("town", ""));

  const parcelRe = parcelRegexRaw ? new RegExp(parcelRegexRaw, "i") : null;
  const propRe = propRegexRaw ? new RegExp(propRegexRaw, "i") : null;
  const addrRe = addrRegexRaw ? new RegExp(addrRegexRaw, "i") : null;
  ...
  console.log("====================================================");
  console.log(" SAMPLE BUCKET RECORDS (NDJSON) v2");
  console.log("====================================================");
  console.log("IN          :", IN);
  console.log("BUCKET      :", bucket);
  console.log("LIMIT       :", limit);
  console.log("missingOnly :", missingOnly);
  if (parcelRegexRaw) console.log("parcelRegex :", parcelRegexRaw);
  if (propRegexRaw) console.log("propertyRegex:", propRegexRaw);
  if (addrRegexRaw) console.log("addrRegex   :", addrRegexRaw);
  if (townFilter) console.log("town        :", townFilter);
  console.log("----------------------------------------------------");
  ...
  for await (const line of rl) {
    ...
    if (townFilter && U(obj.town) !== townFilter) continue;

    const pid = s(obj.parcel_id);
    const prop = s(obj.property_id);
    const full = s(obj.full_address);

    if (parcelRe && !parcelRe.test(pid)) continue;
    if (propRe && !propRe.test(prop)) continue;
    if (addrRe && !addrRe.test(full)) continue;
...
main().catch((e) => {
  console.error("❌ sampleBucket_v2 failed:", e);
  process.exit(1);
});
