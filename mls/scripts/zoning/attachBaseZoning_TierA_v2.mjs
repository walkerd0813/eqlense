...
const TIER_FIELD = args.tierField;     // REQUIRED (institutional)
const TIER_VALUE = args.tierValue;     // REQUIRED (institutional)

const CELL_DEG = Number(args.cellDeg || 0.02);

if (!IN || !OUT || !META || !ZONING_FILE || !TIER_FIELD || !TIER_VALUE) {
  console.error("Usage: node attachBaseZoning_TierA_v2.mjs --in <ndjson> --out <ndjson> --meta <json> --zoningFile <geojson> --tierField <field> --tierValue <value> [--cellDeg 0.02]");
  process.exit(1);
}
...
for (const f of (zoningGj?.features || [])) {
  ...
  const bb = bbox(f);
  const rec = { bbox: bb, geometry: f.geometry, label: pickLabel(f.properties || {}) };
...
console.log("====================================================");
console.log("ATTACH BASE ZONING (Tier A gate: explicit)");
console.log("====================================================");
console.log("IN:", IN);
console.log("OUT:", OUT);
...
console.log("tierField:", TIER_FIELD);
console.log("tierValue:", TIER_VALUE);
console.log("----------------------------------------------------");
...
let total = 0, gatedOut = 0, noCoords = 0, tierA = 0;
...
for await (const line of rl) {
  ...
  const tv = row?.[TIER_FIELD];
  if (String(tv ?? "").trim() !== String(TIER_VALUE).trim()) {
    gatedOut++;
    outRow.zoning.base = { status: "gated_out", tierField: TIER_FIELD, tierValue: tv ?? null, join_method: "pip" };
    ws.write(JSON.stringify(outRow) + "\n");
    continue;
  }
...
const meta = {
  ...
  join: { type: "base_zoning", method: "point_in_polygon", tier_gate: { field: TIER_FIELD, value: TIER_VALUE }, cellDeg: CELL_DEG },
  inputs: { in: IN, zoning_file: ZONING_FILE, zoning_sha256: zoningHash },
  counts: { total_rows: total, tierA_rows: tierA, gated_out: gatedOut, no_coords: noCoords, attached, no_match: noMatch, multi_hit: multiHit },
};
