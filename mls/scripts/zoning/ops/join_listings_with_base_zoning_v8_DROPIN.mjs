...
function normalizePropertyId(pid) {
  ...
  // normalize MA:PARCEL / ma:parcel variants
  s = s.replace(/^ma:parcel:/i, "ma:parcel:");

  // If it's a bare parcel id (no colon), promote it
  // Examples: 0604214000, 117-3, 1990530002900000
...
function extractTown(o) {
  return (
    ...
    o?.link?.town ||
    o?.link?.city ||
    o?.link?.jurisdiction_name ||   // some of your files use this
    o?.raw?.Town ||
    o?.raw?.TOWN ||
    o?.raw?.CITY ||
    ""
...
async function main() {
  ...
  console.log("====================================================");
  console.log("[START] Join MLS listings with Base Zoning v8 (robust key normalization)");
  ...
  for await (const line of rl) {
    ...
    // pull property_id from multiple places and normalize
    const pidRaw = o.property_id || o?.link?.property_id || o?.link?.propertyId || "";
    const pid = normalizePropertyId(pidRaw);

    // allow existing property_uid if present; otherwise compute from town + parcel id
    ...
    if (uid && byUid.has(uid)) {
      ...
      if (cands.length === 0) {
        ...
        if (townMatches.length === 1) {
          ...
          // 4) safe attach if all candidates identical zoning
