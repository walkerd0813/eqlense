...
async function main() {
  ...
  if (fs.existsSync(freezeManifestPath)) {
    const m = readJson(freezeManifestPath); // BOM-safe
  ...
  const uidSet = new Set(); // uniqueness in UID space
  const pidSet = new Set(); // uniqueness in property_id space
