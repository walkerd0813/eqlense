...
function firstNonEmptyRaw(...vals) {
  for (const v of vals) {
    ...
    // don't stringify objects here — that’s how you get "[object Object]"
  }
  return null;
}
...
function extractStringFromMaybeObject(v) {
  ...
  if (typeof v === "object") {
    const keys = ["property_id","propertyId","id","value","pid","parcel_id","parcelId","map_par_id","MAP_PAR_ID"];
    for (const k of keys) {
...
function extractListingPropertyUid(listing) {
  ...
  const maybe = extractStringFromMaybeObject(firstNonEmptyRaw(listing.property_id, getDeep(listing, ["link","property_id"])));
  const parsed2 = parsePropertyUidMaybe(maybe);
...
function normalizePropertyId(s) {
  ...
  // if it looks like a parcel id (digits, hyphens)
...
async function loadUidIndex(indexDir) {
  ...
  const byPropertyId = new Map();   // normalized property_id -> record OR array
...
function attachDefaults(o, asOf) {
  ...
  if (!("base_zone_join_key" in o)) o.base_zone_join_key = "none"; // uid | property_id | parcel_id | ambiguous | none
}
...
async function main() {
  ...
  console.log("====================================================");
  console.log("[START] Join MLS listings with Base Zoning (UID index) v5");
  ...
  let joinByUid = 0, joinByPropertyId = 0, joinByParcelId = 0, ambiguousResolved = 0;
  ...
  for await (const line of rl) {
    ...
    if (pid) {
      ...
      // collision: try disambiguate
      ambiguous++;
      const jKey = normJurisdictionKey(deriveJurisdictionName(o));
      let pick = null;
      if (jKey) pick = cand.find(r => String(r.property_uid || "").startsWith(jKey + "|")) || null;

      if (pick) {
        applyZoning(o, pick, "property_id", asOf);
        found++; joinByPropertyId++; ambiguousResolved++;
      } else {
    ...
    // 3) parcel_id fallback (if present)
    const parcelId = extractStringFromMaybeObject(firstNonEmptyRaw(getDeep(o, ["link","parcel_id"]), getDeep(o, ["link","parcelId"])));
    if (parcelId) {
      const pid2 = normalizePropertyId(parcelId);
      const cand2 = idx.byPropertyId.get(pid2) || null;
      if (cand2 && !Array.isArray(cand2)) {
        applyZoning(o, cand2, "parcel_id", asOf);
        found++; joinByParcelId++;
      } else {
        notFound++;
      }
      out.write(JSON.stringify(o) + "\n");
  ...
  console.log(`[DONE] lines=${lines.toLocaleString()} found=${found.toLocaleString()} notFound=${notFound.toLocaleString()} missingKey=${missingKey.toLocaleString()} ambiguousPid=${ambiguous.toLocaleString()} ambiguousResolved=${ambiguousResolved.toLocaleString()} parseErr=${parseErr.toLocaleString()}`);
