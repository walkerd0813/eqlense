...
const IN = path.resolve(
  __dirname,
  getArg("in", "../../publicData/properties/properties_statewide_geo_zip_district_v13_coords.ndjson")
);
const ADDRESS_INDEX = path.resolve(
  __dirname,
...
const OUT = path.resolve(
  __dirname,
  getArg("out", "../../publicData/properties/properties_statewide_geo_zip_district_v14_coords.ndjson")
);
...
if (path.resolve(IN).toLowerCase() === path.resolve(OUT).toLowerCase()) {
  ...
  ${IN}
Set --out to a different filename (e.g., v14).`
  );
  process.exit(1);
}
...
async function main() {
  console.log("====================================================");
  console.log(" PATCH MISSING COORDS FROM ADDRESS INDEX (v21)");
  console.log("====================================================");
  console.log("IN :", IN);
...
main().catch(e => { console.error("❌ v16 patch failed:", e); process.exit(1); });