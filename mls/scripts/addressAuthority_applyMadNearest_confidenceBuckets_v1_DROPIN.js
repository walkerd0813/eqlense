/**
 * addressAuthority_applyMadNearest_confidenceBuckets_v1_DROPIN.js (ESM)
 *
 * Institutional-style nearest matching:
 *   - SAME-TOWN guard (required when both towns exist)
 *   - ZIP guard (required for matches >60m when both zips exist)
 *   - Street-name guard (if the row already has a street_name, enforce on >60m; warn on <=60m)
 *   - Confidence scoring + distance-bucket labeling
 *   - Leaves an audit trail on every patched row (distance, guards, confidence, bucket)
 *
 * Tile mapping:
 *   - Does NOT depend on tile filename patterns.
 *   - Builds tile map by sampling the first valid point in each tile and computing (ix,iy) from lon/lat.
 *
 * Usage:
 *   node .\mls\scripts\addressAuthority_applyMadNearest_confidenceBuckets_v1_DROPIN.js `
 *     --in <in.ndjson> `
 ...
 *   --tileSize 0.01
 *   --cacheLimit 60
 *   --farZipGuardM 60          (default 60)  -> require zip match for matches beyond this distance
 *   --rejectScoreBelow 40      (default 40)  -> do not patch if confidence would fall below this
 *
 * Behavior:
 *   - Patches ONLY rows that are missing street_no and/or street_name.
 *   - Never overwrites present values.
 *   - Adds fields on patched rows:
 *       addr_authority_src, addr_authority_dist_m,
 *       addr_authority_confidence_score, addr_authority_confidence_bucket,
 *       addr_authority_distance_bucket, addr_authority_guards (object), addr_authority_notes (array)
 */
...
function normalizeStreetNameForCompare(v) {
  ...
  // Normalize common suffixes (last token only)
  const parts = s.split(" ");
  if (parts.length >= 2) {
...
function confidenceScore({ distM, zipMismatch, streetMismatch, missingTown, multiClose }) {
  let score = 100;
...
console.log("===============================================");
console.log(" MAD Nearest (missing-only) + Confidence Buckets");
console.log("===============================================");
...
console.log("farZipGuardM:", farZipGuardM);
console.log("rejectScoreBelow:", rejectScoreBelow);
...
const bandCounts = { A_0_30m: 0, B_30_60m: 0, C_60_120m: 0, D_120_180m: 0 };
...
splitter.on("data", (line) => {
  ...
  const rowTownRaw = row.town ?? row.TOWN ?? row.municipality ?? row.MUNICIPALITY ?? "";
  const rowTown = normTown(rowTownRaw);

  const rowZip = normZip5(row.zip ?? row.ZIP ?? null);
  const rowStreetNameCmp = normalizeStreetNameForCompare(
    row.street_name ?? row.streetName ?? row.STREET_NAME ?? null
  );

  let best = null;
  let bestD = Infinity;
  let closeWithin20m = 0;
  ...
  for (const p of candidates) {
    ...
    const townOk = (!rowTown || !ptown) ? true : (rowTown === ptown);
    if (!townOk) continue;
    ...
    if (d > farZipGuardM && streetMismatch) continue;

    if (d <= 20) closeWithin20m++;
    if (d < bestD) {
      bestD = d;
      best = p;
    }
  ...
  if (!best) {
    // best-effort: detect reject reasons
    let hadTownMismatch = false;
    ...
    for (const p of candidates) {
      ...
      const ptown = normTown(getMadTown(p));
      const townOk = (!rowTown || !ptown) ? true : (rowTown === ptown);
      if (!townOk) { hadTownMismatch = true; continue; }
      ...
      const pStreetCmp = normalizeStreetNameForCompare(getMadStreetName(p));
      const streetMismatch = (rowStreetNameCmp && pStreetCmp) ? (rowStreetNameCmp !== pStreetCmp) : false;
      if (d > farZipGuardM && streetMismatch) { hadStreetMismatch = true; continue; }
    }

    if (hadTownMismatch) rejectedTownMismatch++;
  ...
  const missingTown = (!rowTown || !pTown);
  const multiClose = closeWithin20m >= 2;

  const score = confidenceScore({
    distM: bestD,
    zipMismatch: (bestD <= farZipGuardM) ? zipMismatchSoft : false,
    streetMismatch: (bestD <= farZipGuardM) ? streetMismatchSoft : false,
    missingTown,
    multiClose,
  });
  ...
  const notes = [];
  const guards = {
    same_town: (!rowTown || !pTown) ? 1 : (rowTown === pTown ? 1 : 0),
    ...
    far_zip_guard_applied: bestD > farZipGuardM ? 1 : 0,
    multi_close_candidates_20m: closeWithin20m,
  };

  if (bestD <= farZipGuardM && zipMismatchSoft) notes.push("ZIP_MISMATCH_WITHIN_GUARD");
  if (bestD <= farZipGuardM && streetMismatchSoft) notes.push("STREET_MISMATCH_WITHIN_GUARD");
  if (multiClose) notes.push("AMBIGUOUS_MULTIPLE_CLOSE_CANDIDATES");
  if (missingTown) notes.push("MISSING_TOWN_FOR_STRICT_GUARD");
  ...
  patched++;
  if (bandCounts[distBand] != null) bandCounts[distBand]++;
  confCounts[scoreBand] = (confCounts[scoreBand] ?? 0) + 1;

  row.addr_authority_src = "MAD_NEAREST_CONFIDENCE_BUCKETS";
  row.addr_authority_dist_m = Math.round(bestD * 10) / 10;
  row.addr_authority_distance_bucket = distBand;
  row.addr_authority_confidence_score = score;
  row.addr_authority_confidence_bucket = scoreBand;
  ...
  if (total % 250000 === 0) {
    console.log(`...processed ${total.toLocaleString()} targets=${targets.toLocaleString()} patched=${patched.toLocaleString()} (HIGH=${confCounts.HIGH.toLocaleString()} MED_HIGH=${confCounts.MED_HIGH.toLocaleString()})`);
  }
});
...
splitter.on("end", () => {
  ...
  const report = {
    ...
    policy: {
      sameTown: "Required when both towns exist",
      zipGuard: `Required when dist > ${farZipGuardM}m and both zips exist`,
      streetGuard: `If row has street_name, required when dist > ${farZipGuardM}m`,
