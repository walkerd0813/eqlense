# PatchC_WireSuffix_NoGoto_PS51SAFE_ASCII.ps1
param(
...
$bak = "$Dst.bak_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_BEFORE_V1_11_SUFFIX_WIRE_C_ASCII_NOGOTO"
...
if($already){
  ...
  $wire += ($bodyIndent + "for v in suffix_tail_variants(core):")
  if($usesAdd){
    $wire += ($bodyIndent + "    add(v, ""suffix_tail"")")
  } elseif($usesOutAppend){
    $wire += ($bodyIndent + "    out.append((v, ""suffix_tail""))")
  } else {
    $wire += ($bodyIndent + "    yield (v, ""suffix_tail"")")
  }
  $wire += @("")
...
# Verify parse + compile (safe quoting for PS5.1)
...
Write-Host ("[done] Patch C ASCII (no goto) complete: " + $Dst)
