...
Write-Host "[start] Hampden STEP 2 v1.2 - Attach events to Property Spine (confidence-gated)" -ForegroundColor Cyan
...
if ([string]::IsNullOrWhiteSpace($OutDir)) { $OutDir = Join-Path $BackendRoot "publicData\registry\hampden\_attached_v1_2" }
if ([string]::IsNullOrWhiteSpace($AuditPath)) { $AuditPath = Join-Path $BackendRoot "publicData\_audit\registry\hampden_step2_attach_audit_v1_2.json" }

# Auto-detect spine (prefer NDJSON if available)
...
$OutPath = Join-Path $OutDir "events_attached_v1_2.ndjson"

$py = Join-Path $BackendRoot "hampden_step2_attach_events_to_property_spine_v1_2.py"
if (-not (Test-Path $py)) {
  Write-Host ("[error] Python script not found: {0}" -f $py) -ForegroundColor Red
  exit 1
}
...
Write-Host "[done] Hampden STEP 2 v1.2 complete." -ForegroundColor Green
Write-Host "[next] If attach still low, open the audit 'samples' section to see exact mismatch reasons." -ForegroundColor Cyan
