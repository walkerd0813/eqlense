...
Write-Host "[start] Hampden STEP 2 v1.1 - Attach events to Property Spine (confidence-gated)" -ForegroundColor Cyan
...
if ([string]::IsNullOrWhiteSpace($OutDir)) { $OutDir = Join-Path $BackendRoot "publicData\registry\hampden\_attached_v1_1" }
if ([string]::IsNullOrWhiteSpace($AuditPath)) { $AuditPath = Join-Path $BackendRoot "publicData\_audit\registry\hampden_step2_attach_audit_v1_1.json" }
...
$OutPath = Join-Path $OutDir "events_attached_v1_1.ndjson"

$py = Join-Path $BackendRoot "hampden_step2_attach_events_to_property_spine_v1_1.py"
if (-not (Test-Path $py)) {
...
Write-Host "[done] Hampden STEP 2 v1.1 complete." -ForegroundColor Green
Write-Host "[next] Review audit JSON; then proceed to Market Radar V1 (Hampden-only)." -ForegroundColor Cyan
