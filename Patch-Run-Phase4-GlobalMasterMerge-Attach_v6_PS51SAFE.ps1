# Patch existing v6 runner with a PS51-safe version (no helper funcs / no stray quotes)
$ErrorActionPreference = "Stop"
$ROOT = (Get-Location).Path
$target = Join-Path $ROOT "Run-Phase4-GlobalMasterMerge-Attach_v6_PS51SAFE.ps1"

$src = @'
# Phase 4 — GLOBAL assessor master merge + PropertySpine attach (v6 runner patched)
$ErrorActionPreference = "Stop"

$ROOT = (Get-Location).Path
$CONFIG = Join-Path $ROOT "phase4_global_master_merge_attach_config_v6.json"

Write-Host "[start] Phase 4 — GLOBAL assessor master merge + PropertySpine attach (v6 runner patched)"
