...
# Promotion targets from registry (runner.promote_targets[])
if($runner.PSObject.Properties.Match("promote_targets").Count -and $null -ne $runner.promote_targets){
  foreach($t in @($runner.promote_targets)){
    if([string]::IsNullOrWhiteSpace([string]$t)){ continue }
    $invoke += @("-PromoteArtifactPaths", [string]$t)
  }
}
