...
# --- NEW: hydrate gate outcomes after run (self-healing reads this file) ---
$hydrate = Join-Path $Root "scripts\governance\Hydrate-GateOutcomesFromRunJournal_v0_1.ps1"
if(Test-Path $hydrate){
  & powershell -ExecutionPolicy Bypass -File $hydrate -Root $Root | Out-Host
} else {
  Write-Host ("[warn] missing hydrator: {0}" -f $hydrate)
}
# ---------------------------------------------------------------------------
