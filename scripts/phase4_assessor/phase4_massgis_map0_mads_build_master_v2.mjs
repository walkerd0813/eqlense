...
const meta = await fetchJson(`${layerUrl.replace(/\/$/,"")}?f=pjson`);
const maxRecordCount = meta?.maxRecordCount || null;
console.log(`[info] layerName: ${meta?.name || "(unknown)"} maxRecordCount=${maxRecordCount}`);

const countUrl = buildQueryUrl(layerUrl, { f:"json", where, returnCountOnly:true });
const countJson = await fetchJson(countUrl);
const total = countJson?.count ?? 0;
console.log(`[info] count=${total}`);

const tag = nowTag();
const outMaster = path.join(outMasterDir, `assessor_master__massgis_statewide__${tag}__MAP0__V2.ndjson`);
