...
function saneField(pathKey, value) {
  ...
  // special sanity rules
  if (pathKey.includes("assessed_") || pathKey.includes("sale_price") || pathKey.includes("value")) {
    const n = toNumber(value);
    if (n === null) return false;
    if (n <= 0) return false;
    if (pathKey.includes("sale_price") && n < 1000) return false; // filter junk 100, 10, etc.
    return true;
  }
  ...
  if (pathKey.includes("date")) {
    return !!parseDate(value);
  }
  return true;
}

function choose(cityVal, massVal, key) {
  // precedence: city_assessor > statewide_parcels > null
  if (saneField(key, cityVal)) return { value: cityVal, source: "city_assessor", flags: [] };
  if (saneField(key, massVal)) return { value: massVal, source: "statewide_parcels", flags: ["FALLBACK_USED"] };
...
async function loadAssessorMasters(root, currentPtrRel) {
  ...
  for (const c of (ptr.cities || [])) {
    ...
    for await (const line of rl) {
      ...
      for (const k of parcelKeyVariants(pid)) m.set(k, rec);
    }
    mapCity.set(city, m);
  }
  return { ptrPath, ptr, mapCity };
}

function buildBestFromAssessor(masterRec, bestTemplateFields) {
  // masterRec is already merged city vs mass fallback in the master; but we still want per-field provenance.
  // We'll treat fields whose source_map says statewide_parcels as fallback.
  const best = {};
  const source_map = {};
  ...
  for (const dotted of bestTemplateFields) {
    const src = (masterRec?.source_map && masterRec.source_map[dotted]) ? masterRec.source_map[dotted] : "unknown";
    const v = masterRec ? masterRec[fieldMap[dotted]] : null;

    const obj = {
      value: v ?? null,
      source: src === "statewide_parcels" ? "statewide_parcels" : (src === "city_assessor" ? "city_assessor" : "unknown"),
      as_of: asOf,
      dataset_hash: datasetHash,
      confidence: confidenceFor(src === "statewide_parcels" ? "statewide_parcels" : (src === "city_assessor" ? "city_assessor" : "unknown")),
      flags: []
    };

    // attach fallback flags
    ...
    // only set best if sane; else null but still track source_map
    if (!saneField(dotted, obj.value)) {
      obj.value = null;
      if (!obj.flags.includes("MISSING")) obj.flags.push("MISSING_OR_UNSANE");
    }
...
const cfgPath = args.config ? path.resolve(args.config) : path.join(root, "phase4_property_assessor_best_config_v1.json");
...
const { ptrPath, ptr, mapCity } = await loadAssessorMasters(root, assessorPtrRel);
...
for await (const line of rl) {
  ...
  const city = (rec.source_city || rec.city || rec.city_norm || "").toString().trim().toLowerCase();
  const pid = rec.parcel_id_norm || rec.parcel_id || rec.parcel_id_raw || null;

  const keyList = parcelKeyVariants(pid);
  const cityMap = mapCity.get(city) || null;

  let master = null;
  if (cityMap) {
    for (const k of keyList) { if (cityMap.has(k)) { master = cityMap.get(k); break; } }
  }
  ...
  // keep existing parcels_statewide_raw if already present
  ...
  if (master) {
    matched += 1;
    out.raw_by_source.assessor_city_raw = master.evidence?.municipal_present ? { source_id: master.evidence?.municipal_source_id, source_path: master.evidence?.municipal_source_path, as_of: master.evidence?.as_of, dataset_hash: master.evidence?.dataset_hash } : null;
    out.raw_by_source.parcels_statewide_assess_raw = master.evidence?.massgis_present ? { layer: master.evidence?.massgis_layer, where: master.evidence?.massgis_where, as_of: master.evidence?.as_of, dataset_hash: master.evidence?.dataset_hash } : null;

    // best_current_attributes
    ...
    unmatched += 1;
    if (!out.best_current_attributes || typeof out.best_current_attributes !== "object") out.best_current_attributes = {};
    out.best_current_attributes.assessor = out.best_current_attributes.assessor || {};
    out.best_current_attributes.assessor_source_map = out.best_current_attributes.assessor_source_map || {};
    out.best_current_attributes.assessor_fallback_fields = out.best_current_attributes.assessor_fallback_fields || [];
  }

  ws.write(JSON.stringify(out) + "\n");
  if (n % 200000 === 0) console.log(`[progress] processed ${n} rows (matched=${matched}, unmatched=${unmatched})`);
}
ws.end();
...
const audit = {
  ...
  stats: { properties_rows: n, matched, unmatched, fallback_any: fallbackAny },
  dataset_hashes: {
    properties_in_sha256: sha256File(inProperties),
    assessor_ptr_sha256: sha256Text(JSON.stringify(ptr))
  },
  rules: {
    precedence: "city_assessor > statewide_parcels > null",
...
// CURRENT pointer for downstream jobs
const currentPtr = path.join(outDir, "CURRENT_PROPERTIES_WITH_ASSESSOR_BEST.json");
writeJSON(currentPtr, { updated_at: new Date().toISOString(), note: "AUTO: Phase4 PropertySpine assessor-best attach v1", out_ndjson: outNd, audit: outAudit });
