...
async function countFeatures(layerUrl, where){
...
const cfgPath=args.config?path.resolve(args.config):path.join(root,"phase4_massgis_map0_mads_config_v4.json");
...
let meta;
try{
  meta = await layerMeta(layerUrl);
}catch(e){
  // if meta fails, try FeatureServer equivalent
  const fsUrl = mapServerToFeatureServer(layerUrl);
  console.log(`[warn] meta failed on MapServer. trying FeatureServer meta: ${fsUrl}`);
  layerUrl = fsUrl;
  meta = await layerMeta(layerUrl);
}
console.log(`[info] layerName: ${meta?.name || "(unknown)"} maxRecordCount=${meta?.maxRecordCount ?? null}`);

let countJson;
try{
  countJson = await countFeatures(layerUrl, where);
}catch(e){
  const fsUrl = mapServerToFeatureServer(layerUrl);
  if(fsUrl !== layerUrl){
    console.log(`[warn] count failed on MapServer. trying FeatureServer: ${fsUrl}`);
    layerUrl = fsUrl;
    countJson = await countFeatures(layerUrl, where);
  } else {
    throw e;
...
if(total<=0){
  throw new Error(`[err] count returned 0. Layer may require different where/filter or is not queryable: ${layerUrl}`);
}

const tag = nowTag();
const outMaster = path.join(outMasterDir, `assessor_master__massgis_statewide__${tag}__MAP0__V4.ndjson`);
...
for(let offset=0; offset<total; offset += pageSize){
  ...
  for(const ft of feats){
    ...
    rec.evidence.dataset_hash = sha256Text(JSON.stringify({
      ...
      schema: "assessor_master_massgis_map0_v4"
    }));
...
const auditPath = path.join(outAuditDir, `phase4_massgis_map0_mads_master__${tag}__V4.json`);
...
writeJSON(currentPtr, {
  ...
  note: "AUTO: MassGIS MAP0 MADS master v4 (MapServer->FeatureServer fallback)",
