...
function upgradeRecord(rec, createdAtIso) {
  ...
  // v4: rename "assessment_year" -> "tax_fy" to avoid confusion with calendar year.
  // Many Massachusetts assessor datasets label fiscal years ahead of the calendar year (e.g., FY2026 values published in late 2025).
  if (best?.valuation?.assessment_year && !best.valuation.tax_fy) {
    best.valuation.tax_fy = best.valuation.tax_fy;
    delete best.valuation.tax_fy;
  }
...
async function main() {
  ...
  // Accept either: node script <configPath>  OR  node script --config <configPath>
  let configArg = argv[0];
  if (configArg === "--config") configArg = argv[1];
  const configPath = path.resolve(ROOT, configArg || "phase4_assessor_best_provenance_upgrade_config_v4.json");
  ...
  const audit = {
    ...
    notes: [
      "v3: treats valuation.tax_fy as Fiscal Year (FY) and also writes valuation.tax_fy as the UI-friendly alias.",
