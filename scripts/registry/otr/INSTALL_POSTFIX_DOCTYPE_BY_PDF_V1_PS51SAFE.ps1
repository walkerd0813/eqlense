...
$py = Join-Path $Root "scripts\registry\otr\otr_postfix_doctype_by_pdf_v1.py"
$ps = Join-Path $Root "scripts\registry\otr\Run-Hampden-OTR-Postfix-DocTypeByPdf-v1_PS51SAFE.ps1"
$runner = Join-Path $Root "scripts\registry\otr\Run-Hampden-OTR-Extract-v1_PS51SAFE.ps1"

New-Item -ItemType Directory -Force -Path (Split-Path $py -Parent) | Out-Null

# ---------- Python: postfix doc_type per PDF (streaming) ----------
$pyText = @"
...
[IO.File]::WriteAllText($py, $pyText, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "[ok] wrote $py"
...
python (Join-Path (Split-Path \$PSScriptRoot -Parent) "otr\otr_postfix_doctype_by_pdf_v1.py") --in_events "\$InEvents" --out_dir "\$OutDir"
"@

[IO.File]::WriteAllText($ps, $psText, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "[ok] wrote $ps"

# ---------- Patch extractor runner to autorun this postfix and update $outEvents ----------
if (-not (Test-Path $runner)) { throw "Runner not found: $runner" }

$txt = Get-Content -Path $runner -Raw -Encoding UTF8
if ($txt -like '*POSTFIX_DOCTYPE_BY_PDF_AUTORUN_V1*') {
  Write-Host "[ok] autorun patch already present (POSTFIX_DOCTYPE_BY_PDF_AUTORUN_V1)"
  exit 0
}

# Insert AFTER existing POSTFIX_DOCTYPES_AUTORUN_V1 block end marker if present, else after param block.
$insertAfter = "# --- /POSTFIX_DOCTYPES_AUTORUN_V1 ---"
$idx = $txt.IndexOf($insertAfter)
if ($idx -ge 0) {
  $idx = $idx + $insertAfter.Length
} else {
  $idx = $txt.IndexOf(")")
  if ($idx -lt 0) { throw "Could not find insertion point in runner." }
  $idx = $idx + 1
}
...
try {
  ...
  \$outDir2 = Join-Path \$root "publicData\registry\hampden\_work\OTR_EXTRACT_ALLDOCS_v1\_postfix_doctype_by_pdf"
  New-Item -ItemType Directory -Force -Path \$outDir2 | Out-Null

  if (\$outEvents -and (Test-Path \$outEvents)) {
    Write-Host "[auto] postfix doc_type by PDF on \$outEvents"
    ...
    \$latest2 = Get-ChildItem -Path \$outDir2 -Filter "events__POSTFIX_DOCTYPE_BY_PDF__*.ndjson" -ErrorAction SilentlyContinue |
      Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if (\$latest2) {
      \$outEvents = \$latest2.FullName
      Write-Host "[auto] outEvents -> \$outEvents"
    }
  } else {
    Write-Host "[auto][skip] outEvents missing; cannot run postfix doc_type by PDF"
  }
} catch {
  Write-Host "[auto][warn] postfix doc_type by PDF failed: \$($_.Exception.Message)"
}
# --- /POSTFIX_DOCTYPE_BY_PDF_AUTORUN_V1 ---
`r`n
"@
...
$bak = "$runner.bak_{0:yyyyMMdd_HHmmss}_DOCTYPE_BY_PDF" -f (Get-Date)
Copy-Item $runner $bak -Force
Set-Content -Path $runner -Value $txt2 -Encoding UTF8

Write-Host "[ok] patched autorun into: $runner"
