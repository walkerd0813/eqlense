...
function Add-LayerHit([string]$city, [string]$rootType, [hashtable]$layer, [System.IO.FileInfo]$fileObj) {
  ...
  foreach ($m in (MatchesAny $hay $include)) { if ($m) { [void]$layerMap[$key].match_keywords.Add($m) } }
  foreach ($m in (MatchesAny $hay $envDup)) { if ($m) { [void]$layerMap[$key].envdup_keywords.Add($m) } }
}
...
foreach ($k in $layerMap.Keys) {
  ...
  $rows.Add([pscustomobject]@{
    ...
    matched_keywords = (($v.match_keywords | Sort-Object) -join ";")
    env_duplicate = $isEnvDup
    envdup_keywords = (($v.envdup_keywords | Sort-Object) -join ";")
