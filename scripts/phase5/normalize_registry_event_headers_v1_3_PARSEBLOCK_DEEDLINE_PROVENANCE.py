...
def main():
    ...
    with open(args.infile,"r",encoding="utf-8") as fin, open(args.out,"w",encoding="utf-8") as fout:
        for line in fin:
            ...
            # keep existing consideration.amount if present
            if amt_int is not None:
                kept_existing += 1
                parse_status = c.get("parse_status") or "PARSED"
                src = src or "existing_consideration"
            else:
                # parse from deed line if available
    ...
    audit = {
        "script": "normalize_registry_event_headers_v1_3_PARSEBLOCK_DEEDLINE_PROVENANCE",
        "created_at_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "infile": args.infile,
        "out": args.out,
        "total": total,
        "written": wrote,
        "kept_existing_consideration": kept_existing,
    ...
    print("rows:", wrote, "kept_existing:", kept_existing, "filled_from_deed_line:", filled_from_deed_line, "missing:", missing, "mirrored:", mirrored)
