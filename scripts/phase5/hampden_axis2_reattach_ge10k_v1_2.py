...
OUT_NDJSON = r"publicData/registry/hampden/_attached_DEED_ONLY_v1_8_1_MULTI/axis2_candidates_ge_10k__reattached_axis2_v1_2.ndjson"
OUT_AUDIT  = r"publicData/_audit/registry/hampden_axis2_reattach_ge10k_v1_2.json"
...
# Canonicalize ALL common suffix variants to abbreviations
SUFFIX_CANON = {
  "STREET":"ST", "ST":"ST",
  "ROAD":"RD", "RD":"RD",
...
def norm_full_address(s: str) -> str:
    # apply suffix canon on the tail token of the whole string (helps full_address matching)
    s = norm_text(s)
    toks = s.split()
    if len(toks) >= 2 and toks[-1] in SUFFIX_CANON:
        toks[-1] = SUFFIX_CANON[toks[-1]]
    elif len(toks) >= 2 and toks[-1] in SUFFIX_CANON.values():
        pass
    return " ".join(toks)
...
def build_spine_indexes(spine_path: str):
    ...
    for row in iter_ndjson(spine_path):
        ...
        if street_no and street_nm:
            ...
            st = canon_suffix_last_token(street_nm)
            ks = f"{town}|{sn}|{st}"
...
def try_match_single(town: str, addr_raw: str, idx_full, idx_unit, idx_street, idx_street_nosuf):
    ...
    # fallback: no-suffix street key, unique only
    st0 = street_nosuf(st)
    ks0 = f"{town}|{sn}|{st0}"
...
def main():
    ...
    with open(OUT_NDJSON, "w", encoding="utf-8") as fout:
        for ev in iter_ndjson(CAND):
            ...
            if scope == "MULTI" and a.get("attachments"):
                ...
                a["evidence"]["join_basis"] = "axis2_suffix_canon_v1_2"
                a["evidence"]["join_method"] = "town+full_address OR town+street_no+street_name(+unit) unique_only (+nosuf fallback unique)"
                fout.write(json.dumps(ev, ensure_ascii=False) + "\n")
                continue
    ...
    audit = {
      ...
      "notes": [
        ...
        "Suffix canon (BOULEVARD->BLVD, DRIVE->DR, AVENUE->AVE, etc).",
        ...
        "Fallback: town+street_no+street_name(without suffix) only if UNIQUE in spine.",
