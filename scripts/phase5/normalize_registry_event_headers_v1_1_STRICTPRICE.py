...
def main():
    ...
    with open(args.infile, "r", encoding="utf-8") as fin, open(args.out, "w", encoding="utf-8") as fout:
        for line in fin:
            ...
            # consideration: STRICT - only trust ts.price_amount (or existing consideration.amount if already set)
            ...
            # text-based flags from raw_block (safe)
            if raw_block:
            ...
            ev["consideration"] = {
                ...
                "flags": flags,
                "source": "transaction_semantics.price_amount" if ts.get("price_amount") is not None else (c.get("source") or "missing")
            }
            ...
            # parties: keep any existing + store raw lines if present
            p = ev.get("parties")
            if not isinstance(p, dict):
                p = {}
            if raw_block and not p.get("party_lines_raw"):
                party_lines = []
                for ln in raw_block.splitlines():
                    ln2 = ln.strip()
                    if re.search(r"^\d+\s+P\s+[A-Z]", ln2):
                        party_lines.append(ln2)
                if party_lines:
    ...
    audit = {
        "script": "normalize_registry_event_headers_v1_1_STRICTPRICE",
        ...
        "missing_consideration_amount": missing_cons,
    ...
    print("rows:", wrote, "cons_amount_filled:", filled_cons, "cons_amount_missing:", missing_cons, "doc_type_filled:", filled_doc)
