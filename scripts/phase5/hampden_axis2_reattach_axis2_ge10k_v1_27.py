...
def set_attach(evt: dict, scope: str, status: str, method: str = None, property_id: str = None,
              why: str = None, match_key: str = None, match_meta: dict = None, evidence: dict = None,
              attachments: list = None):
    """
    Canonical attach lives ONLY at evt["attach"].
    We also remove/avoid top-level duplicates to prevent drift.
    """
    a = evt.get("attach")
    ...
    a["attach_scope"] = scope
    a["attach_status"] = status
    a["match_method"] = method
    a["property_id"] = property_id
    a["why"] = why
    if match_key is not None:
        a["match_key"] = match_key
    if match_meta is not None:
        a["match_meta"] = match_meta
    if evidence is not None:
        a["evidence"] = evidence
    if attachments is not None:
        a["attachments"] = attachments

    # Strip duplicated top-level fields (if present)
    for k in ("attach_scope","attach_status","why","match_method","property_id","match_meta"):
        if k in evt:
            evt.pop(k, None)
...
def main():
    ...
    audit = {
        "run": {"script": os.path.basename(__file__), "version": "v1_27", "ran_at": datetime.now(timezone.utc).isoformat()},
        "inputs": {
