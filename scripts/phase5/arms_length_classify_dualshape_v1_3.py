...
def get_doc_text(ev):
    ...
    # Legacy Hampden shape: use document fields
    doc = ev.get("document") or {}
    legacy = " | ".join([
        norm_str(doc.get("index_doc_type")),
        norm_str(doc.get("instrument_type")),
        norm_str(doc.get("document_type")),
        norm_str(doc.get("raw_block"))[:2000]  # keep bounded
    ]).strip()

    return legacy

def get_cons(ev):
    # Prefer MIM consideration.amount
    ...
    # Legacy fallback: transaction_semantics.price_amount
    if amt is None:
        ts = ev.get("transaction_semantics") or {}
        amt = to_int(ts.get("price_amount"))
        if amt is not None and parse_status == "MISSING":
            parse_status = "PARSED"

    return amt, parse_status, flags, raw_text

def get_attach(ev):
    # Prefer MIM attach.status/property_id
    a = ev.get("attach") or {}
    status = a.get("status")
    ...
    return status, pid, (a.get("confidence") if isinstance(a, dict) else None)
...
def party_stats(ev):
    ...
    # Case 2: Legacy deed shape: parties.grantor/grantee arrays
    if not isinstance(parties, list):
        p = parties if isinstance(parties, dict) else {}
        grantors = p.get("grantor")
        grantees = p.get("grantee")
        if isinstance(grantors, list):
            roles["GRANTOR"] += len(grantors)
            has_grantor = True if len(grantors) > 0 else has_grantor
            for nm in grantors:
                if not has_gov and text_has_any(str(nm or ""), GOV_HINTS):
                    has_gov = True
        if isinstance(grantees, list):
            roles["GRANTEE"] += len(grantees)
            has_grantee = True if len(grantees) > 0 else has_grantee
            for nm in grantees:
                if not has_gov and text_has_any(str(nm or ""), GOV_HINTS):
                    has_gov = True

    # Case 3: parties_raw list
    pr = ev.get("parties_raw")
    if isinstance(pr, list) and not roles:
        roles["UNKNOWN"] += len(pr)
        for nm in pr[:10]:
            if not has_gov and text_has_any(str(nm or ""), GOV_HINTS):
                has_gov = True
...
def classify_one(ev, min_amount):
    ...
    # ---- missing/ambiguous consideration => UNKNOWN ----
    if cls == "UNKNOWN":
...
def main():
    ...
    with open(args.infile, "r", encoding="utf-8") as fin, open(args.out, "w", encoding="utf-8") as fout:
        for line in fin:
            ...
            # classify deed-like events here
    ...
    audit = {
        "script": "arms_length_classify_dualshape_v1_3",
