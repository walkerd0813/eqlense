...
DEFAULT_IN = r"publicData/registry/hampden/_attached_DEED_ONLY_v1_8_1_MULTI/axis2_candidates_ge_10k__reattached_axis2_v1_12.ndjson"
DEFAULT_OUT = r"publicData/registry/hampden/_attached_DEED_ONLY_v1_8_1_MULTI/axis2_candidates_ge_10k__reattached_axis2_v1_15.ndjson"
DEFAULT_AUDIT = r"publicData/_audit/registry/hampden_axis2_reattach_ge10k_v1_15.json"
...
def build_target_keys(events):
    ...
    for ev in events:
        a = ev.get("attach") or {}
        st = (a.get("attach_status") or "").upper()
        scope = (a.get("attach_scope") or "").upper()
        if scope == "MULTI" or st == "PARTIAL_MULTI":
            continue
...
def main():
    ...
    for ev in events:
        ...
        # ✅ CRITICAL: preserve MULTI rows; never flatten them
        if scope == "MULTI" or st == "PARTIAL_MULTI":
            # still optional: record docno segmentation meta for audit-only
            ...
            if meta.get("seg_applied"):
                seg_stats["seg_applied_multi"] += 1
                seg_stats["seg_applied_multi_docno"] += 1
                ev.setdefault("meta", {})
        ...
        # preserve already-attached SINGLE rows
        if st and st != "UNKNOWN":
            stats["already_attached_or_not_unknown"] += 1
            out_rows.append(ev)
            continue

        # Only SINGLE UNKNOWN rows get reattach
        rb = ((ev.get("document") or {}).get("raw_block")) or ""
        docno_raw = ((ev.get("recording") or {}).get("document_number_raw")) or ""
        seg, meta = segment_by_docno(rb, str(docno_raw))
        if meta.get("seg_applied"):
            seg_stats["seg_applied_single"] += 1
            town_seg, addr_seg = extract_town_addr_from_segment(seg)
            if town_seg and addr_seg:
                seg_stats["seg_extracted_town_addr_single"] += 1
                pr = ev.get("property_ref") or {}
        ...
        if pid:
            ev["attach"].update({
                ...
                "evidence":{
                    ...
                    "join_basis":"axis2_docno_segment_v1_15_preserve_multi",
    ...
    os.makedirs(os.path.dirname(args.auditp), exist_ok=True)
    audit = {
        "in":args.inp,"out":args.outp,
        "spine_current":SPINE_CURRENT_JSON,
        "spine_path":spine_path,
        "stats":dict(stats),
    ...
    print("=== AXIS2 REATTACH (>=10k) v1_15 (PRESERVE MULTI) ===")
    print(json.dumps({"out":args.outp,"audit":args.auditp,"spine_path":spine_path,"stats":dict(stats),"seg_stats":dict(seg_stats)}, indent=2))
