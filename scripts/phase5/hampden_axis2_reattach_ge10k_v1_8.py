...
def segment_by_dash(raw_block: str, town0: str, addr0: str, docno_raw: str|None):
    ...
    if not DASH_DELIM.search(rb):
        return rb, {"segmented": False, "why":"no_dash_delim", "chunks":1, "match_chunks":0}

    chunks = [c for c in DASH_DELIM.split(rb) if c.strip()]
    match = []

    # strongest: Town+Addr evidence
    town_tag = f"TOWN: {town0}"
    addr_tag = f"ADDR:{addr0}"
    for c in chunks:
        cu = c.upper()
        if (town0 and addr0) and (town_tag in cu) and (addr_tag in cu):
