...
START = "# ------------------------ Address Variant Helpers ------------------------"
END   = "# ---------------------- End Address Variant Helpers ----------------------"

NEW_BLOCK = r"""# ------------------------ Address Variant Helpers ------------------------
...
def main():
    ...
    bak = PY + ".bak_helpers_v1_7_18_" + ts
    shutil.copy2(PY, bak)
    print("[backup]", bak)
    ...
    if a == -1 or b == -1 or b < a:
        print("[fail] helper markers not found or malformed in file")
        sys.exit(2)
    b_end = b + len(END)

    patched = src[:a] + NEW_BLOCK + src[b_end:]
    open(PY, "w", encoding="utf-8").write(patched)
    print("[ok] helper block replaced via string markers")
    print("[done] v1.7.18 helpers patch applied")
