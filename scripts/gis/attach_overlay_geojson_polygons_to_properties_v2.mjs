...
async function loadFeaturesGeoJSONSeq(filePath){
  const feats = [];
  ...
  for await (const line of rl){
    ...
    if (f && f.type === "Feature" && f.geometry) feats.push(f);
  }
  return feats;
}
...
async function run(){
  ...
  const ext = path.extname(geoPath).toLowerCase();
  const isSeq = (ext === ".geojsons" || ext === ".geojsonseq" || ext === ".jsons");

  let features = [];
  if (isSeq) {
    features = await loadFeaturesGeoJSONSeq(geoPath);
  } else {
    // For small files only
    const gj = JSON.parse(fs.readFileSync(geoPath, "utf8"));
    features = Array.isArray(gj?.features) ? gj.features.filter(f => f && f.geometry) : [];
  }
  ...
  const manifest = {
    ...
    inputs: {
      ...
      format: isSeq ? "GeoJSONSeq" : "GeoJSON"
    },
    outputs: {
