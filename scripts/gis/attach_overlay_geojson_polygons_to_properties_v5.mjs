...
async function run(){
  ...
  for await (const line of rl){
    if (!line) continue;
    propsRead++; if (propsRead % 50000 === 0) { console.log(`[prog] props_read=${propsRead}`); } if (propsRead % 200000 === 0) { console.log(`[prog] props_read=${propsRead} props_with_any=${propsWithAny} attachments=${attaches}`); } let obj;
