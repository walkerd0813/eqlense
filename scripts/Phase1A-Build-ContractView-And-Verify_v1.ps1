...
if (!(Test-Path ".\scripts\Phase1A-Finish-And-Verify_v4_1.ps1")) { throw "Missing verify script: .\scripts\Phase1A-Finish-And-Verify_v4_1.ps1" }

# Write node builder (created every run to prevent “missing file” errors)
$nodePath = ".\scripts\gis\build_property_contract_view_v1.mjs"
New-Item -ItemType Directory -Force -Path (Split-Path $nodePath) | Out-Null
...
function norm(x){ if(x===undefined||x===null) return ""; return String(x).trim(); }
...
for await (const line of rl){
  ...
  // Centroid fallbacks (exists as headers even if null)
  ...
  // Base zoning (headers must exist; UNKNOWN allowed)
  const base_zoning_code_raw =
    pick(row,["base_zoning_code_raw","baseZoningCodeRaw","zoning_code_raw","zoning","zone"]) || "";
  const base_zoning_code_norm =
    pick(row,["base_zoning_code_norm","baseZoningCodeNorm","zoning_code_norm","zone_norm"]) || base_zoning_code_raw;
...
$hash = (Get-FileHash $PropertiesNdjson -Algorithm SHA256).Hash
...
Write-Host "[run] build contract view -> $outNdjson"
node $nodePath --in $PropertiesNdjson --out $outNdjson --datasetHash $hash --asOfDate $AsOfDate
if ($LASTEXITCODE -ne 0) { throw "node contract view failed exit=$LASTEXITCODE" }

Write-Host ""
Write-Host "[run] verify CONTRACT VIEW (not frozen spine)"
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Phase1A-Finish-And-Verify_v4_1.ps1 `
