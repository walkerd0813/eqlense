...
# Pointer may target either a FILE (preferred) or a FREEZE DIRECTORY.
if (!(Test-Path $contractIn)) { throw "Contract view not found: $contractIn" }
if (Test-Path $contractIn -PathType Container) {
  $nd = Get-ChildItem -Path $contractIn -File -Filter "*.ndjson" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
  if (-not $nd) { throw "Contract view directory has no .ndjson file: $contractIn" }
