...
if(-not (Test-Path $CoverageCsv)){
  throw "CoverageCsv not found. Provide -CoverageCsv explicitly."
}
...
Log "====================================================="
Log "[START] Build Zoning Capability Matrix (PS5.1)"
...
$cityCounts = @{}  # key: city/town folder name

$cityDirs = Get-ChildItem $ZoningRoot -Directory -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -notmatch "^\_" }  # skip _normalized, _misc, etc

foreach($d in $cityDirs){
  $city = $d.Name.ToLower()

  $districts = @(Get-ChildItem $d.FullName -Recurse -File -Filter "*.geojson" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "\\districts\\" }).Count

  $overlays = @(Get-ChildItem $d.FullName -Recurse -File -Filter "*.geojson" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "\\overlays\\" }).Count

  $subdistricts = @(Get-ChildItem $d.FullName -Recurse -File -Filter "*.geojson" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "\\subdistricts\\" }).Count

  $misc = @(Get-ChildItem $d.FullName -Recurse -File -Filter "*.geojson" -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match "\\_misc\\" }).Count

  $total = @(Get-ChildItem $d.FullName -Recurse -File -Filter "*.geojson" -ErrorAction SilentlyContinue).Count
  ...
  $cityCounts[$city] = [pscustomobject]@{
    ...
    subdistrictsFiles = $subdistricts
    miscFiles = $misc
  }
}
...
# 2) Load coverage CSV (output-driven truth)
...
foreach($r in $cov){
  ...
  if($cityCounts.ContainsKey($town)){
    $has = $true
    $cc = $cityCounts[$town]
  }
...
# 4) Backlog: not harvested, ranked by volume
...
# 5) “Weird” list: harvested but baseRatePct==0 (signals bad layer / wrong field / not polygon coverage)
...
Log ("[OK ] wrote harvested-but-zero-base list: {0}" -f $outWeird)
