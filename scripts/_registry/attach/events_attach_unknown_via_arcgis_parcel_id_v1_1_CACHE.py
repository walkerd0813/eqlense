...
# leading number or range: 400-402, 154R, 12A
...
def main():
    ...
    # result = {"hit": bool, "attrs": attrs_or_none}
    cache = {}
    ...
    with open(args.events, "r", encoding="utf-8") as fi, open(args.out, "w", encoding="utf-8") as fo:
        for ln in fi:
            ...
            if qkey in cache:
                ...
                if not feats:
                    res = {"hit": False, "attrs": None}
                    c["arc_no_hit"] += 1
                else:
                    attrs = (feats[0].get("attributes") or {})
                    # normalize minimal evidence
                    res = {"hit": True, "attrs": attrs}
                    c["arc_hit"] += 1
            ...
            # Always record ArcGIS evidence if we had a hit (even if not attachable)
            ...
            # Attach deterministically via parcel-id
            a["property_id"] = pid
            a["status"] = "ATTACHED_A"
