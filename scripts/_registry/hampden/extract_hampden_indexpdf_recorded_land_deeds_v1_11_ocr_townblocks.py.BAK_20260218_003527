...
def extract_events_from_lines(lines: List[str], run_id: str, page_index: int) -> List[Dict[str, Any]]:
    ...
    appear to the LEFT of the "FILE SIMPLIFILE" marker; we capture them as pending
    and attach them to the next record when we see FILE SIMPLIFILE.
    """
    ...
    cur = new_cur()
    started = False  # ignore page header until first FILE SIMPLIFILE marker
    ...
    def maybe_capture_grp_seq(line: str) -> bool:
        ...
        right after the row context line and before FILE SIMPLIFILE.
        """
    ...
    for ln in lines:
        ...
        # Before started: ignore headers, but capture left-column context if present
        if not started:
            if maybe_capture_row_context(ln):
                continue
            if maybe_capture_grp_seq(ln):
                continue
            if up.startswith("FILE SIMPLIFILE"):
                started = True
        ...
        # After started: we still might see row-context lines before each FILE SIMPLIFILE
        if maybe_capture_row_context(ln):
            continue
        if maybe_capture_grp_seq(ln):
            continue

        # Record separator (start of new record)
        if up.startswith("FILE SIMPLIFILE"):
            # If we accidentally have residue, flush it
        ...
        if "DEED" in up:
            ...
            # Consideration: allow down to $1, only after DEED
            if not cons:
            ...
            if descr_loc:
                # If we already have content from a prior record (refs/parties/descr),
                # a new descr line usually means a new record boundary.
                if has_any_record_content(cur) and (cur.get("property_refs") or cur.get("parties_raw") or cur.get("descr_loc")):
                    flush()
        ...
        if "TOWN:" in up and "ADDR:" in up:
            ...
            if town or addr:
                # If we already have parties, this is likely the next transaction block
                # (but if we have only refs so far, keep collecting refs for the same transaction)
                if cur.get("parties_raw") and (cur.get("property_refs") or cur.get("descr_loc")):
                    flush()
