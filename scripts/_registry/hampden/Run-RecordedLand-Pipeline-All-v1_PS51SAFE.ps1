...
if ([string]::IsNullOrWhiteSpace($TownBlocksPy)) {
  $TownBlocksPy = Join-Path $Root "scripts\_registry\hampden\extract_hampden_indexpdf_recorded_land_deeds_v1_11_ocr_townblocks.py"
}
...
if ([string]::IsNullOrWhiteSpace($StitchPy)) {
  $StitchPy = Join-Path $Root "scripts\_registry\hampden\stitch_townblocks_pagebreak_continuations_v1.py"
}
...
for ($p = $StartPage; $p -lt $EndPageExclusive; $p += $ChunkSize) {
  ...
  $tbOut = Join-Path $chunkDir ("events__HAMPDEN__RECORDED_LAND__" + $DocType.ToUpper() + "__v1_11__" + $pTag + ".ndjson")
  $tbAudit = Join-Path $chunkDir ("audit__TB__" + $pTag + "__v1_11.json")
  $tbQuar = Join-Path $chunkDir ("quarantine__TB__" + $pTag + "__v1_11")
  $tbRawLines = Join-Path $tbQuar "raw_ocr_lines__ALLPAGES.ndjson"

  $tbStitched = Join-Path $chunkDir ("events__HAMPDEN__RECORDED_LAND__" + $DocType.ToUpper() + "__v1_11__" + $pTag + "__STITCHED_v1.ndjson")
  ...
  & python $StitchPy --in "$tbOut" --raw_lines_ndjson "$tbRawLines" --out "$tbStitched" --qa "$tbStitchQa"
  if ($LASTEXITCODE -ne 0) { throw ("Stitch failed exit=" + $LASTEXITCODE + " chunk=" + $pTag) }

  # 3) RowCtx (v1_19_8) — uses page_start/page_end (end exclusive)
  ...
  # 4) Join (v1_3_1) — join stitched townblocks with rowctx
