#!/usr/bin/env python3
"""
stitch_townblocks_pagebreak_continuations_v1_5_2.py
...
def extract_top_continuation(lines_raw: List[str], max_scan: int = 250) -> Tuple[List[Tuple[Optional[str], str]], List[dict], List[str]]:
    ...
    for s in captured:
        if not (RX_TOWN.search(s) and RX_ADDR.search(s)):
            continue
    ...
    # 4) extract parties conservatively
...
def main():
    ...
    counts = {
        ...
        "notes": "v1_5_2 stitches refs+parties from top-of-next-page raw OCR lines; supports --raw and --raw_lines_ndjson."
    }
    samples: List[dict] = []
    ...
    with open(args.qa_path, "w", encoding="utf-8") as f:
        json.dump({
            "engine": "stitch_townblocks_pagebreak_continuations_v1_5_2",
