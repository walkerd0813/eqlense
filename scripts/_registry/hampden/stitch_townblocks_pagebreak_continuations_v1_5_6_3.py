...
def safe_stitch_event(base, last_ev, *, from_page, into_page, refs_found, parties_found, captured, counts, samples, continuation_type: str):
    """
    Call base stitch_event in a backwards-compatible way (older frozen stitchers
    may not accept continuation_type kwarg). Never recurse.
    """
    stitch_fn = getattr(base, "stitch_event", None)
    if stitch_fn is None:
        raise RuntimeError("base module missing stitch_event")

    # Try with continuation_type kwarg
    try:
        stitch_fn(
            last_ev,
            from_page=from_page,
    ...
    # Record type ourselves for QA/debug
    try:
...
def main() -> None:
    ...
    for p in pages:
        ...
        if same_refs:
            ...
            safe_stitch_event(base, last_ev,
                from_page=p,
                into_page=p,
                refs_found=refs_found,
                parties_found=parties_found,
        ...
        safe_stitch_event(base, last_ev,
            ...
            captured=captured,
            counts=counts,
            samples=samples, continuation_type="PAGEBREAK"
        )
