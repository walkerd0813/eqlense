#!/usr/bin/env python3
"""
stitch_townblocks_pagebreak_continuations_v1_5_5_1.py
...
# --- v1_5: continuation detection improvements (header-skip + deeper scan) ---
RX_TOWN = re.compile(r'\\bTown\\s*:\\s*', re.I)
RX_ADDR = re.compile(r'\\bAddr\\s*:\\s*', re.I)

def _is_header_noise_line(s: str) -> bool:
    if not s: return True
    t = s.strip().upper()
    if t.startswith('RECORDED LAND BY RECORDING DATE'): return True
    if 'INQUIRY PRINT REQUEST' in t: return True
    if t.startswith('PRINTED '): return True
    if t.startswith('TOWN: *ALL'): return True
    if t == 'PAGE': return True
    if t.startswith('CONSIDERATION:'): return True
    if 'TRANSACTION #:' in t: return True
    if 'SEQ DESCR/LOC/' in t and 'BOOK-' in t and 'CONSIDERATION' in t: return True
    return False

def _find_first_town_idx(lines_raw, max_scan=250):
    n = min(max_scan, len(lines_raw))
    for i in range(n):
        s = (lines_raw[i] or '').strip()
        if _is_header_noise_line(s):
            continue
        if RX_TOWN.search(s):
            return i
    return None

def _capture_continuation_block(lines_raw, start_i, max_block=140):
    out = []
    saw_first_town = False
    end = min(len(lines_raw), start_i + max_block)
...
# -------------------------
def extract_top_continuation(lines_raw: List[str], max_scan: int = 250) -> Tuple[List[Tuple[Optional[str], str]], List[dict], List[str]]:
    import re
    RX_TOWN = re.compile(r'\bTown\s*:\s*', re.I)
    RX_ADDR = re.compile(r'\bAddr\s*:\s*', re.I)
    ...
    def is_header_noise(s: str) -> bool:
        if not s:
            return True
        t = s.strip().upper()
        # v1_5_4: ignore the '*ALL' header town line (often OCRs as *ALIL, *AL1L, etc.)
        if t.startswith('TOWN:') and '*' in t: return True
        if t.startswith('RECORDED LAND BY RECORDING DATE'): return True
        if 'INQUIRY PRINT REQUEST' in t: return True
        if t.startswith('PRINTED '): return True
        if t.startswith('TOWN: *ALL'): return True
    ...
    # 1) find first Town line beyond header noise
    n = min(max_scan, len(lines_raw))
    start_i: Optional[int] = None
    for i in range(n):
        s = (lines_raw[i] or '').strip()
        if is_header_noise(s):
            continue
        if RX_TOWN.search(s):
            start_i = i
            break
    ...
    # 2) capture until next Town line (exclusive)
    captured: List[str] = []
    saw_first_town = False
    end = min(len(lines_raw), start_i + 220)
    for j in range(start_i, end):
        s = (lines_raw[j] or '').strip()
        if not s:
            continue
        if RX_TOWN.search(s):
            if saw_first_town:
    ...
    # 3) extract refs as (town, addr_raw)
    refs_found: List[Tuple[Optional[str], str]] = []
    pending_town: Optional[str] = None
    for s in captured:
        # Case 1: Town + Addr on same line
        if RX_TOWN.search(s) and RX_ADDR.search(s):
            m_t = re.search(r'(?i)Town:\\s*([^A]+?)\\s+Addr:', s)
            m_a = re.search(r'(?i)Addr:\\s*(.+?)\\s*(?:Y|$)', s)
            town = (m_t.group(1).strip() if m_t else None)
            addr = (m_a.group(1).strip() if m_a else s)
            refs_found.append((town, addr))
            pending_town = None
            continue
    
        # Case 2: Town only (store pending)
        if RX_TOWN.search(s) and (not RX_ADDR.search(s)):
            mt = re.search(r'(?i)Town:\\s*(.+?)\\s*(?:Y|$)', s)
            pending_town = (mt.group(1).strip() if mt else None)
            continue
    
        # Case 3: Addr only (pair with pending Town)
        if RX_ADDR.search(s) and (not RX_TOWN.search(s)) and pending_town:
            ma = re.search(r'(?i)Addr:\\s*(.+?)\\s*(?:Y|$)', s)
            addr = (ma.group(1).strip() if ma else s)
            refs_found.append((pending_town, addr))
            pending_town = None
            continue
    ...
    for s in captured:
        ...
        if u.startswith('FILE '):
            parties_found.append({'role': 'FILE', 'name': s.strip()})
            continue
        pm = re.match(r'^\d+\s+([PC])\s+(.+?)(?:\s+Y)?$', s.strip(), flags=re.I)
        if pm:
            parties_found.append({'role': pm.group(1).upper(), 'name': pm.group(2).strip()})
...
def main():
    ...
    counts = {
        ...
        "notes": "v1_5_5_1 stitches refs+parties; supports parties-only continuations from top-of-next-page raw OCR lines; supports --raw and --raw_lines_ndjson."
    }
    samples: List[dict] = []
    ...
    for p in pages:
        ...
        # v1_5_5_1: parties-only continuation (e.g., 'SEE RECORD' rows where address is missing in source index)
        if parties_found:
            try:
                if last_ev.get('parties') is None:
                    last_ev['parties'] = {}
                if last_ev['parties'].get('parties_raw') is None:
                    last_ev['parties']['parties_raw'] = []
                for pty in parties_found:
                    nm = (pty.get('name') or '').strip()
                    if not nm: continue
                    role = (pty.get('role') or '').strip()
                    last_ev['parties']['parties_raw'].append((role + ' ' + nm).strip())
                if last_ev.get('meta') is None:
                    last_ev['meta'] = {}
                last_ev['meta']['stitch'] = {
                    'from_page': p,
                    'into_page': next_p,
                    'continuation_type': 'PARTIES_ONLY',
                    'addr_missing_in_source': True
                }
            except Exception:
                pass
    ...
    with open(args.qa_path, "w", encoding="utf-8") as f:
        json.dump({
            "engine": "stitch_townblocks_pagebreak_continuations_v1_5_5_1",
