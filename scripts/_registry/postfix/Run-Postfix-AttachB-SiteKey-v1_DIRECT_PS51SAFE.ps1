...
Write-Host "[start] ATTACHED_B via site_key (DIRECT python)"
...
python -m py_compile $script | Out-Null

python $script --infile "$InFile" --spine "$Spine" --out "$out" --audit "$audit" --engine_id "postfix.unknown_to_attachedB_sitekey_v1"
