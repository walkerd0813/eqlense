...
function classify(layer) {
  ...
  // Stronger sub-type scoring (to encourage 4 per subject being truly different)
  const score = (s) => {
    const t = h;
    let p = 0;
    if (type === "water") {
      if (t.includes("main") || t.includes("watermain")) p += 4;
      if (t.includes("hydrant")) p += 4;
      if (t.includes("valve") || t.includes("gate")) p += 3;
      if (t.includes("service") || t.includes("lateral") || t.includes("curb stop")) p += 2;
    } else if (type === "sewer") {
      if (t.includes("force") || t.includes("forcemain")) p += 4;
      if (t.includes("gravity") || t.includes("main")) p += 3;
      if (t.includes("manhole")) p += 4;
      if (t.includes("lateral") || t.includes("service") || t.includes("connection")) p += 2;
    } else if (type === "storm") {
      if (t.includes("catch basin") || t.includes("catchbasin") || t.includes("inlet")) p += 4;
      if (t.includes("outfall")) p += 4;
      if (t.includes("drain") || t.includes("drainage") || t.includes("main")) p += 3;
      if (t.includes("culvert")) p += 2;
    }
    if (isNetwork) p += 2;
    if (isNoise) p -= 10;
  ...
  return { hay, type, isNoise, isAdj, isNetwork, suspicious, score: score(hay) };
}
...
for (const layer of layers) {
  ...
  const k = city;
  if (!byCityBuckets.has(k)) byCityBuckets.set(k, { water: [], sewer: [], storm: [], unknown: [] });
  byCityBuckets.get(k)[c.type].push({ layer, score: c.score, hay: c.hay, type: c.type, cls: c });
}

const auditPath = path.join(auditDir, `phase3_utilities_FINAL_audit__${nowTag()}.json`);
writeJSON(auditPath, audit);

// --- PRUNE: select top N per type per city, excluding noise + zero-feature + adjacent-without-network
const quarantine = [];
const kept = [];
const dropped = [];
  ...
  for (const type of ["water","sewer","storm"]) {
    const arr = buckets[type]
      .filter(x => {
        ...
        // must be network OR at least not suspicious; prefer network
    ...
    // quarantine: zero-feature layers
    ...
    // dropped are those not selected (for report)
...
// Build FINAL dict object, preserving metadata
const finalDict = {
  ...dict,
  created_at: new Date().toISOString(),
...
const relFinal = finalDictPath; // keep absolute for clarity in your system
writeJSON(currentPtr, { path: relFinal, updated_at: new Date().toISOString(), note: "AUTO: Phase 3 FINAL pack v2" });

if (fs.existsSync(contractPtr)) {
  const contract = readJSON(contractPtr);
  contract.phase3_utilities = relFinal;
  contract.updated_at = new Date().toISOString();
  writeJSON(contractPtr, contract);
}

// Prune report + signoff
const pruneReport = {
  created_at: new Date().toISOString(),
  source_dict_path: absDictPath,
  source_dict_sha256: dictHash,
  ...
  quarantine: quarantine.slice(0, 1000),
  dropped: dropped.slice(0, 1000)
};
...
const signoff = {
  ...
  pointers: {
    ...
    CURRENT_CONTRACT_VIEW_MA: contractPtr
  },
  dict: {
    final_dict_path: finalDictPath,
...
console.log("[done] Phase 3 FINAL v2 complete.");
