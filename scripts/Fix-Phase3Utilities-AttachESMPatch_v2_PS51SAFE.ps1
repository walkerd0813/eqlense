...
Write-Host "===================================================="
Write-Host "PHASE 3 — UTILITIES ATTACH ESM PATCH (PS 5.1 SAFE) v2"
Write-Host "===================================================="
...
$hasRequire = ($src -match "require\(['""]node:readline['""]\)")
if (-not $hasRequire) {
...
$hasImport = ($src -match "^\s*import\s+readline\s+from\s+['""]node:readline['""]\s*;?\s*$"m)

# Replace require(...).createInterface( with readline.createInterface(
$patched = $src
$patched = [regex]::Replace($patched, "require\(['""]node:readline['""]\)\.createInterface\s*\(", "readline.createInterface(")
