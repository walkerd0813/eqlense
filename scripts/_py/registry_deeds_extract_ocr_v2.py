...
def parse_fields(text: str):
    ...
    # Recorded date (prefer labeled, fallback any date)
    ...
    def block_after(labels):
        for lab in labels:
            m = re.search(rf"\b{re.escape(lab)}\b\s*[:\-]?\s*([\s\S]{{0,400}})", t, re.I)
            if m:
                b = m.group(1)
                stop = re.search(r"\b(grantor|grantee|consideration|recorded|book|page|doc|instrument|property address|address)\b", b, re.I)
                if stop:
                    b = b[:stop.start()]
                return b.strip()
        return None
...
def ocr_pdf_first_pages(pdf_path: str, page_limit: int, dpi: int):
    ...
    for i in range(n):
        ...
        if len(txt) > 50000:
            txt = txt[:50000]
        total_len += len(txt)
        pages.append({"page_index": i, "ocr_text_len": len(txt), "ocr_text": txt})

    doc.close()
    avg = (total_len / max(1, len(pages)))
    return pages, total_len, avg
...
def main():
    ...
    for i, fn in enumerate(pdfs, start=1):
        ...
        try:
            pages, total_len, avg = ocr_pdf_first_pages(p, args.pageLimit, args.dpi)
            merged = "\n".join([x["ocr_text"] for x in pages])
            ...
            rec = {
                ...
                "source": {
                    ...
                    "extract_method": "ocr_tesseract_v2",
                    "ocr_page_limit": args.pageLimit,
                    "ocr_dpi": args.dpi,
