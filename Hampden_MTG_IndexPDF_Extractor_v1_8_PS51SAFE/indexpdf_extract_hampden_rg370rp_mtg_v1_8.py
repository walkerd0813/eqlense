...
def extract_words_in_body(page):
    """
    Extract words from the body region, excluding header/footer.
    Hampden RG370RP PDFs sometimes encode text as single-character glyphs.
    We retry with looser tolerances if we detect "character soup".
    """
    h = page.height
    bbox = (0, h * 0.20, page.width, h * 0.94)

    def _words(x_tol, y_tol, flow):
        return page.within_bbox(bbox).extract_words(
    ...
    words = _words(3.0, 3.0, True)

    # Detect single-character tokenization (bad grouping)
    if words:
        one = sum(1 for w in words if len((w.get("text") or "")) == 1)
        if (one / max(1, len(words))) > 0.60 and len(words) > 200:
            # retry: wider grouping window
            words = _words(6.0, 3.5, True)
...
def row_text(row: List[Dict[str, Any]]) -> str:
    """Reconstruct a readable line from clustered word tokens."""
    toks = [w.get('text') for w in row if w.get('text')]
    if not toks:
        return ''
    t = ' '.join(toks)
    # collapse digit soup and separator spacing
    t = re.sub(r'(?<=\d)\s+(?=\d)', '', t)            # 0 1 -> 01 (but may also join date+time; we fix below)
    t = re.sub(r'\s*-\s*', '-', t)
    t = re.sub(r'\s*/\s*', '/', t)
    t = re.sub(r'\s*:\s*', ':', t)
    t = re.sub(r'\s+([ap])\b', r'\1', t, flags=re.I)  # '39 p' -> '39p'
    # ensure date/time boundary has a space
    t = re.sub(r'(\d{2}-\d{2}-\d{4})(\d{1,2}:)', r'\1 \2', t)
