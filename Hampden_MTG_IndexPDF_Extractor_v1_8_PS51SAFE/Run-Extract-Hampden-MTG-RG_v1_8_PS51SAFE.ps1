...
$py = Join-Path $Root "scripts\_registry\hampden\indexpdf_extract_hampden_rg370rp_mtg_v1_8.py"

function Now-RunId {
  $t = (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
  return "${t}__hampden__indexpdf_v1_8"
}

if ([string]::IsNullOrWhiteSpace($RunId)) { $RunId = Now-RunId }

$work = Join-Path $Root ("publicData\registry\hampden\_work\INDEXPDF_EXTRACT_v1_8\" + $RunId)
...
Write-Host "[start] Hampden RG370RP MTG extract v1_8"
