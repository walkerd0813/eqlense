# PatchC_WireSuffix_NoLabels_PS51SAFE.ps1
param(
...
$bak = "$Dst.bak_" + (Get-Date -Format "yyyyMMdd_HHmmss") + "_BEFORE_V1_11_SUFFIX_WIRE_C_NO_LABELS"
Copy-Item $Dst $bak -Force
Write-Host "[backup] $bak"

$lines = Get-Content $Dst -Encoding UTF8

# 0) Sanity: suffix engine must exist
$hasSuffix = $false
foreach($ln in $lines){
...
# 2) Determine function body indent
$bodyIndent = "    "
...
# 3) Function end = next top-level def
$endIdx = $lines.Count
...
for($k = $defIdx; $k -lt $endIdx; $k++){
  if($lines[$k] -match 'suffix_tail_variants\s*\(\s*core\s*\)'){ $already = $true; break }
}
if($already){
  Write-Host "[skip] already wired in addr_variants()" -ForegroundColor DarkYellow
} else {

  # 5) Find best insertion point: last "core" assignment/cleanup
  $coreLineIdx = -1
  ...
  if($coreLineIdx -ge 0){
    ...
    # fallback after docstring end if present (""" only)
    $docEnd = -1
    $inDoc = $false
    for($k = $defIdx + 1; $k -lt $endIdx; $k++){
      $t = $lines[$k]
      if($t -match '"""'){
        if(-not $inDoc){ $inDoc = $true } else { $docEnd = $k; break }
      }
    }
    if($docEnd -ge 0){ $insertAt = $docEnd + 1 }
    else {
      for($k = $defIdx + 1; $k -lt $endIdx; $k++){
  ...
  if($usesAdd){
    ...
    $emitLines = @(
      "${bodyIndent}# NOTE: addr_variants emission mechanism not detected; no-op wire.",
      "${bodyIndent}# Search inside addr_variants and tell ChatGPT what structure it uses (add/out.append/yield)."
    )
  }
...
Write-Host "[done] Patch C (no labels) complete: $Dst"
