...
Say "[start] Install: OpsJournal + AcceptGate scaffolding v0_1_1 (PS5.1-safe)"
Say "  root:   $Root"
Say "  dryrun: $DryRun"
...
$readme = @'
OpsJournal + AcceptGate (v0_1_1) — what this gives you
-----------------------------------------------------
...
$toWrite = @(
  @{ path = (Join-Path $opsDir "Run-OpsJournal-Start_v0_1_PS51SAFE.ps1"); content = $runStartPs1 },
  @{ path = (Join-Path $opsDir "Run-OpsJournal-End_v0_1_PS51SAFE.ps1"); content = $runEndPs1 },
  @{ path = (Join-Path $opsDir "Bless-Script_v0_1_PS51SAFE.ps1"); content = $blessPs1 },
  @{ path = (Join-Path $opsDir "README__OpsJournal_AcceptGate_v0_1_1.txt"); content = $readme }
)

foreach ($f in $toWrite) {
  if ($DryRun) { Say "[dryrun] would write $($f.path)" }
  else { WriteUtf8NoBom $f.path $f.content; Say "[ok] wrote $($f.path)" }
}
