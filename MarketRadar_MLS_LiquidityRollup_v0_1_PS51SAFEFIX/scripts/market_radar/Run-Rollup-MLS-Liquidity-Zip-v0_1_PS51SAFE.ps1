...
Write-Host "[start] rollup MLS LIQUIDITY -> ZIP (v0_1)..."
...
$py = Join-Path $PSScriptRoot "rollup_mls_liquidity_zip_v0_1.py"
$cmd = "python `"$py`" --infile `"$Infile`" --out `"$Out`" --audit `"$Audit`" --as_of `"$AsOf`" --windows `"$Windows`""
Write-Host ("[run] {0}" -f $cmd)

python $py --infile $Infile --out $Out --audit $Audit --as_of $AsOf --windows $Windows
if ($LASTEXITCODE -ne 0) { throw "[error] MLS liquidity rollup failed with exit code $LASTEXITCODE" }

Write-Host "[done] MLS liquidity rollup complete."
